...
export const SideNav = memo(() => {
    ...
    // Listen for toggle event from navbar burger button
    useEffect(() => {
        const handleToggle = () => {
            setIsOpen(prev => !prev);
        };
    ...
    useEffect(() => {
        const handleClickOutside = (e: MouseEvent) => {
            ...
            const trigger = document.querySelector('[data-sidenav-trigger]');

            if (isOpen &&
                sidenav && !sidenav.contains(e.target as Node) &&
    ...
    return (
        <aside className={`max-w-62.5 ease-nav-brand z-990 fixed inset-y-0 my-4 ml-4 block w-full flex-wrap items-center justify-between rounded-2xl border-0 bg-white p-0 antialiased shadow-none transition-transform duration-200 xl:left-0 xl:translate-x-0 xl:bg-transparent ${isOpen ? 'translate-x-0 shadow-soft-xl' : '-translate-x-full'}`}>
            ...
            >
                ...
                <div className="sticky top-0 z-20 max-xl:bg-white min-[1200px]:bg-gray-50">
                    <div className="h-19.5 px-6 py-6">
                        <FontAwesomeIcon
                            icon={faTimes}
                            className={`absolute top-0 right-0 p-4 opacity-50 cursor-pointer text-slate-400 xl:hidden ${isOpen ? 'block' : 'hidden'}`}
                            onClick={handleClose}
                        />
                        <Link
